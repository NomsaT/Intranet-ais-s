...
export class UnitOfMeasurementComponent implements OnInit {
  ...
  statData: Array<[]>;
  isActive: string;
  pageName: string = "Unit Of Measurement";
  event: any;
  CancelDownload = true;
      ...
      exportDataGrid({
        ...
        workbook.xlsx.writeBuffer()
          .then(function (buffer: BlobPart) {
            saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'Unit-Of-Measurement-List.xlsx');
          });
      });
    }
  };
  ...
  constructor(@Inject('BASE_URL') baseUrl: string, private modalService: NgbModal, private globalMethodsService: GlobalMethodsService, private authService: AuthenticationService) {
    this.dataSource = {
      store: AspNetData.createStore({
        ...
        updateUrl: baseUrl + 'api/UnitOfMeasurement',
        insertUrl: baseUrl + 'api/UnitOfMeasurement',
        deleteUrl: baseUrl + 'api/UnitOfMeasurement',
        onInserted: () => {
    ...
    this.allowModify = this.authService.HavePermission(43);
  }
