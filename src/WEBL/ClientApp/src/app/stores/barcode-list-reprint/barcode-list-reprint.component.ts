...
@Component({
  selector: 'app-barcode-list-reprint',
  templateUrl: './barcode-list-reprint.component.html',
  styleUrls: ['./barcode-list-reprint.component.scss']
})
export class ReprintBarcodeListComponent implements OnInit {
  dataSource: any;
  isVisible: string;
  transactions: Array<[]>;
  statData: Array<[]>;
  ...
  constructor(@Inject('BASE_URL') baseUrl: string, private modalService: NgbModal, private globalMethodsService: GlobalMethodsService, private authService: AuthenticationService, private barcodesService: BarcodesService) {
    this.dataSource = {
      store: AspNetData.createStore({
        key: 'id',
        loadUrl: baseUrl + 'api/PrintBarcode/getReprintBarcode'
    ...
    this.allowModify = this.authService.HavePermission(98);
  }
