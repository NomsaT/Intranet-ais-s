...
@Component({
  selector: 'app-grouping-stock',
  templateUrl: './grouping-stock.component.html',
  styleUrls: ['./grouping-stock.component.scss']
})
export class GroupingStockComponent implements AfterViewInit {
  dataSource: any;
      ...
      exportDataGrid({
        ...
        workbook.xlsx.writeBuffer()
          .then(function (buffer: BlobPart) {
            saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'Group-Stock-List.xlsx');
          });
      });
    }
  };
  ...
  constructor(@Inject('BASE_URL') baseUrl: string, private modalService: NgbModal, private globalMethodsService: GlobalMethodsService) {
    this.currencyZAR = currencyZAR;
    this.dataSource = {
      store: AspNetData.createStore({
        key: 'id',
        loadUrl: baseUrl + 'api/GroupingStock'
