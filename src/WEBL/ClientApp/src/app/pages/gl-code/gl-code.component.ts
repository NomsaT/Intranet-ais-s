      ...
      exportDataGrid({
        ...
        workbook.xlsx.writeBuffer()
          .then(function (buffer: BlobPart) {
            saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'GL-Code-List.xlsx');
          });
      });
    }
  };
  ...
  constructor(@Inject('BASE_URL') baseUrl: string, private modalService: NgbModal, private globalMethodsService: GlobalMethodsService, private authService: AuthenticationService) {
    this.dataSource = {
      store: AspNetData.createStore({
        ...
        updateUrl: baseUrl + 'api/GLCodes',
        insertUrl: baseUrl + 'api/GLCodes',
        deleteUrl: baseUrl + 'api/GLCodes',
        onInserted: () => {
    ...
    this.allowModify = this.authService.HavePermission(75);
  }
