      ...
      exportDataGrid({
        ...
        workbook.xlsx.writeBuffer()
          .then(function (buffer: BlobPart) {
            saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'Stock-Group-List.xlsx');
          });
      });
    }
  };
  ...
  constructor(@Inject('BASE_URL') baseUrl: string, private modalService: NgbModal, private globalMethodsService: GlobalMethodsService, private authService: AuthenticationService) {
    this.dataSource = {
      store: AspNetData.createStore({
        ...
        updateUrl: baseUrl + 'api/StockGroup',
        insertUrl: baseUrl + 'api/StockGroup',
        deleteUrl: baseUrl + 'api/StockGroup',
        onInserted: () => {
    ...
    this.allowModify = this.authService.HavePermission(52);
  }
