...
export class PaymentMethodComponent implements OnInit {
  ...
  statData: Array<[]>;
  isActive: string;
  pageName: string = "Payment Method";
  event: any;
  CancelDownload = true;
      ...
      exportDataGrid({
        ...
        workbook.xlsx.writeBuffer()
          .then(function (buffer: BlobPart) {
            saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'Payment-Methods-List.xlsx');
          });
      });
    }
  };
  ...
  constructor(@Inject('BASE_URL') baseUrl: string, private modalService: NgbModal, private globalMethodsService: GlobalMethodsService, private authService: AuthenticationService) {
    this.dataSource = {
      store: AspNetData.createStore({
        ...
        updateUrl: baseUrl + 'api/PaymentMethod',
        insertUrl: baseUrl + 'api/PaymentMethod',
        deleteUrl: baseUrl + 'api/PaymentMethod',
        onInserted: () => {
    ...
    this.allowModify = this.authService.HavePermission(45);
  }
  ...
  newRow(e) {
    e.data.requireBankingDetails = false;
  }
}
