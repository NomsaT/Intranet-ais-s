...
@Component({
  ...
  styleUrls: ['./cost-types.component.scss']
})
      ...
      exportDataGrid({
        ...
        workbook.xlsx.writeBuffer()
          .then(function (buffer: BlobPart) {
            saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'Cost-Type-List.xlsx');
          });
      });
    }
  };
  ...
  constructor(@Inject('BASE_URL') baseUrl: string, private modalService: NgbModal, private globalMethodsService: GlobalMethodsService, private authService: AuthenticationService) {
    this.dataSource = {
      store: AspNetData.createStore({
        ...
        updateUrl: baseUrl + 'api/CostTypes',
        insertUrl: baseUrl + 'api/CostTypes',
        deleteUrl: baseUrl + 'api/CostTypes',
        onInserted: () => {
    ...
    this.allowModify = this.authService.HavePermission(77);
  }
