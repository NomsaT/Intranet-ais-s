...
@Component({
  selector: 'app-gl-vat-report',
  templateUrl: './gl-vat-report.component.html',
  styleUrls: ['./gl-vat-report.component.scss']
})
export class GLVATReportComponent implements OnInit {
  dataSource: any;
      ...
      exportDataGrid({
        ...
        workbook.xlsx.writeBuffer()
          .then(function (buffer: BlobPart) {
            saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'GL-VAT-Report-List.xlsx');
          });
      });
    }
  };
  ...
  constructor(@Inject('BASE_URL') baseUrl: string, private modalService: NgbModal) {
    ...
    this.GLCodeForeignDataSource = {
      store: AspNetData.createStore({
        key: 'id',
        loadUrl: baseUrl + 'api/VatStored/getglcode',
      }),
      paginate: true
    }
  }
  ...
  randFormatConversion(cellInfo) {
    if (cellInfo.value)
      return GlobalMethodsService.formatCurrency(cellInfo.value, "");
    else
