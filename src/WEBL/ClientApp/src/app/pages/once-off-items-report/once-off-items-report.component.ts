...
@Component({
  selector: 'app-once-off-items-report',
  templateUrl: './once-off-items-report.component.html',
  styleUrls: ['./once-off-items-report.component.scss']
})
export class OnceOffItemsReportComponent implements OnInit {
  dataSource: any;
      ...
      exportDataGrid({
        ...
        workbook.xlsx.writeBuffer()
          .then(function (buffer: BlobPart) {
            saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'Once-Off-Report-List.xlsx');
          });
      });
    }
  };
  ...
  constructor(@Inject('BASE_URL') baseUrl: string, private modalService: NgbModal) {
    ...
    this.dataSource = {
      store: AspNetData.createStore({
        key: 'id',
        loadUrl: baseUrl + 'api/OnceOffReport',
