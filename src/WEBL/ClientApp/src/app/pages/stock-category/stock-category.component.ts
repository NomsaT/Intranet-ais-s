      ...
      exportDataGrid({
        ...
        workbook.xlsx.writeBuffer()
          .then(function (buffer: BlobPart) {
            saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'Stock-Category-List.xlsx');
          });
      });
    }
  };
  ...
  constructor(@Inject('BASE_URL') baseUrl: string, private modalService: NgbModal, private globalMethodsService: GlobalMethodsService, private authService: AuthenticationService) {
    this.dataSource = {
      store: AspNetData.createStore({
        ...
        updateUrl: baseUrl + 'api/StockCategory',
        insertUrl: baseUrl + 'api/StockCategory',
        deleteUrl: baseUrl + 'api/StockCategory',
        onInserted: () => {
    ...
    this.allowModify = this.authService.HavePermission(41);
  }
